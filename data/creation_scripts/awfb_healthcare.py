"""
AW_FB Healthcare Dataset Creation Script

This script processes AW_FB CSV files (activity, calories)
...
Usage:
    ...
    python data/creation_scripts/awfb_healthcare.py --sys_prompt tabc

Tasks:
    - activity: Multi-class classification 
      (Self Pace Walk, Sitting, Lying, Running 7 METs, Running 5 METs, Running 3 METs)
    - calories: Regression (calorie burn prediction)
"""
...
CSV_FILES = {
    "activity": "/home/fzha836/HealthLLM-RLCR/data/AW_FB/AW_FB_activity.csv",
    "calories": "/home/fzha836/HealthLLM-RLCR/data/AW_FB/AW_FB_calories.csv",
}
...
def load_single_csv(csv_path, train_ratio=0.8, seed=42):
    ...
    # (activity has string answers, calories has float answers)
...
def load_all_csv_files(csv_files, train_ratio=0.8, seed=42):
...
def create_dataset_for_prompt(raw_dataset, sys_prompt_name, output_dir):
    ...
    # Save
    save_path = os.path.join(output_dir, f"awfb_{sys_prompt_name}")
    processed.save_to_disk(save_path)
    
    return processed, save_path


def create_datasets_for_all_prompts(csv_files, output_dir, train_ratio=0.8, seed=42):
    """
...
def create_dataset_for_single_prompt(csv_files, output_dir, sys_prompt_name, train_ratio=0.8, seed=42):
...
def main():
    parser = argparse.ArgumentParser(description="Process AW_FB healthcare CSV files")
    ...
    parser.add_argument("--hub_name", type=str, default=None,
                        help="HuggingFace Hub dataset name prefix (e.g., 'username/awfb')")
    
    args = parser.parse_args()
    
    print(f"\n{'='*70}")
    print("AW_FB Healthcare Dataset Processing")
    print(f"{'='*70}")
    ...
    print(f"\nTasks:")
    print(f"  - activity: Multi-class classification")
    print(f"    (Self Pace Walk, Sitting, Lying, Running 7/5/3 METs)")
    print(f"  - calories: Regression (calorie burn prediction)")
    ...
    if args.sys_prompt == "all":
        ...
        for prompt_name in ALL_SYS_PROMPTS:
            print(f"  - {args.output_dir}/awfb_{prompt_name}/")
        
        print("\nTo load a dataset:")
        print("  from datasets import load_from_disk")
        print(f"  dataset = load_from_disk('{args.output_dir}/awfb_tabc')")
    else:
        print(f"\nTo load the dataset:")
        print("  from datasets import load_from_disk")
        print(f"  dataset = load_from_disk('{args.output_dir}/awfb_{args.sys_prompt}')")
