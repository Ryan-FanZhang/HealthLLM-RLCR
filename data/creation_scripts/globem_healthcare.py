"""
GLOBEM Healthcare Dataset Creation Script

This script processes GLOBEM CSV files (anxiety, depression)
...
Usage:
    ...
    python data/creation_scripts/globem_healthcare.py --sys_prompt tabc

Tasks:
    - anxiety: PHQ-4 anxiety score (0 to 4)
    - depression: PHQ-4 depression score (0 to 4)
"""
...
CSV_FILES = {
    "anxiety": "/home/fzha836/HealthLLM-RLCR/data/GLOBEM/GLOBEM_anxiety.csv",
    "depression": "/home/fzha836/HealthLLM-RLCR/data/GLOBEM/GLOBEM_depression.csv",
}
...
def create_dataset_for_prompt(raw_dataset, sys_prompt_name, output_dir):
    ...
    # Save
    save_path = os.path.join(output_dir, f"globem_{sys_prompt_name}")
...
def main():
    parser = argparse.ArgumentParser(description="Process GLOBEM healthcare CSV files")
    ...
    parser.add_argument("--hub_name", type=str, default=None,
                        help="HuggingFace Hub dataset name prefix (e.g., 'username/globem')")
    
    args = parser.parse_args()
    
    print(f"\n{'='*70}")
    print("GLOBEM Healthcare Dataset Processing")
    print(f"{'='*70}")
    ...
    print(f"\nTasks:")
    print(f"  - anxiety: PHQ-4 anxiety score (0 to 4)")
    print(f"  - depression: PHQ-4 depression score (0 to 4)")
    ...
    if args.sys_prompt == "all":
        ...
        for prompt_name in ALL_SYS_PROMPTS:
            print(f"  - {args.output_dir}/globem_{prompt_name}/")
        
        print("\nTo load a dataset:")
        print("  from datasets import load_from_disk")
        print(f"  dataset = load_from_disk('{args.output_dir}/globem_tabc')")
    else:
        print(f"\nTo load the dataset:")
        print("  from datasets import load_from_disk")
        print(f"  dataset = load_from_disk('{args.output_dir}/globem_{args.sys_prompt}')")
